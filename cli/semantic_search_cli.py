...
def main():
    ...
    verify_parser = subparsers.add_parser(
        "verify", help="Verify the model of the semantic search"
    )
    embedText_parser = subparsers.add_parser(
        "embed_text", help="Embed text using the semantic search model"
    )
    verifyEmbeddings_parser = subparsers.add_parser(
        "verify_embeddings", help="Verify the embeddings of the semantic search model"
    )
    embedQuery_parser = subparsers.add_parser(
        "embedquery", help="Embed query using the semantic search model"
    )
    search_parser = subparsers.add_parser(
        "search", help="Search using the semantic search model"
    )
    chunk_parser = subparsers.add_parser(
        "chunk", help="Chunk text using the semantic model"
    )
    semanticChunk_parser = subparsers.add_parser(
        "semantic_chunk", help="Chunk text using the semantic model"
    )
